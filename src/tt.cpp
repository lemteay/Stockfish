...
void TTEntry::save(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8) {
    ...
    if (value16 != VALUE_NONE && eval16 != VALUE_NONE)
    {
        complexityBonus = (std::abs(value16 - eval16) + 128) / 512;
        complexityBonus = std::min(complexityBonus, 4);
    }

    // Overwrite less valuable entries (cheapest checks first)
    if (b == BOUND_EXACT || uint16_t(k) != key16
        || d - DEPTH_ENTRY_OFFSET + 2 * pv > depth8 - 4 + complexityBonus
